...
theta_phi_kde:
    ...
    data,                       theta_weights, phi_weights
bm1 [all measurements of bm1]
bm2
bm3
.
...
from .fast_kde import compute_ln_likelihood_kde_fast
import re 
import os 
import logging 
...
def get_initial_kde_estimates(
    ...
    for bm in range(N):
        ...
        _,_, is_theta  = get_two_clusters_with_kmeans(
...
def compute_unbiased_stage_likelihoods_kde(
    ...
    for participant in range(n_participants):
        ...
        for k_j in range(n_stages):
            ln_stage_likelihoods[k_j] = compute_ln_likelihood_kde_fast(
                n_participants=n_participants,
