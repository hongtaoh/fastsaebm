import numpy as np
import fastsaebm.utils as utils 
...
def metropolis_hastings(
    ...
    if algorithm == 'kde':
        # N * 3 * J matrix, each row: biomarker; cols: data, theta_weights, phi_weights; each cell is a J-length vector
        theta_phi_default = utils.get_initial_kde_estimates(
    ...
    for iteration in range(iterations):
        ...
        if algorithm not in ['hard_kmeans']:
            ...
            if algorithm == 'kde':
                new_theta_phi = utils.update_theta_phi_estimates_kde(
                    n_biomarkers,
                    n_participants,
                    non_diseased_ids,
                    data_matrix,
